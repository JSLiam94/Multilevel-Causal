...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        if self.hybrid:
            self.hybrid_model = ResNetV2(block_units=self.config.resnet.num_layers, width_factor=self.config.resnet.width_factor)
            in_channels = self.hybrid_model.width * 16
        self.patch_embeddings = Conv2d(in_channels=in_channels,
                                       out_channels=self.config.hidden_size,
                                       kernel_size=patch_size,
                                       stride=patch_size)
        self.position_embeddings = nn.Parameter(torch.zeros(1, n_patches+self.config.n_classes, config.hidden_size))#change(?)
        self.cls_token = nn.Parameter(torch.zeros(1, self.config.n_classes, self.config.hidden_size))#change(?)
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        for layer_block in self.layer:
            ...
            hidden_embed.append(hidden_states)
            if self.vis:
                attn_weights.append(weights)
        encoded = self.encoder_norm(hidden_states)
        return encoded, hidden_embed #attn_weights
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        return encoded, attn_weights, features#return encoded, attn_weights, features
...
class VisionTransformerres32fu(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        self.config = config
        self.n_lyer=self.config.transformer["num_layers"]
